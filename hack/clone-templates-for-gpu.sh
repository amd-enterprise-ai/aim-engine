...
# spec.gpuSelector.model matches PROFILE_GPU, clones them with the
# target GPU model, and outputs the new manifests to stdout.
#
...
MATCHING_COUNT=$(echo "$TEMPLATES" | jq -r --arg gpu "$PROFILE_GPU" '[.items[] | select(.spec.gpuSelector.model == $gpu)] | length')
...
MODELS=$(echo "$TEMPLATES" | jq -r --arg gpu "$PROFILE_GPU" '[.items[] | select(.spec.gpuSelector.model == $gpu) | .spec.modelName] | unique | .[]')
...
while IFS= read -r MODEL_NAME; do
    ...
    while IFS= read -r template; do
        ...
        YAML=$(echo "$template" | jq \
            ...
            # Update GPU selector
            .spec.gpuSelector.model = $targetGpu |
...
$YAML"
        ...
        '.items[] | select(.spec.gpuSelector.model == $gpu and .spec.modelName == $model)')
done <<< "$MODELS"
